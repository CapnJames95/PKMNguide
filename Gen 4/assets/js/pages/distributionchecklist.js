function buildDistributionChecklistPage() {
  ...
  function renderChecklist() {
    ...
    function currentGameLabel() {
      ...
      return ({ all: 'All Games', FR: 'Diamond', LG: 'Pearl', R: 'Platinum', S: 'HeartGold', E: 'SoulSilver' }[game]) || game;
    }
    ...
    function gameMatches(entry) {
      ...
      return (entry.targets || []).some(function(target) {
        var t = norm(target);
        if (selectedGame === 'FR') return t.indexOf('diamond') !== -1 || t === 'd';
        if (selectedGame === 'LG') return t.indexOf('pearl') !== -1 || t === 'p';
        if (selectedGame === 'R') return t.indexOf('platinum') !== -1 || t === 'pt';
        if (selectedGame === 'S') return t.indexOf('heartgold') !== -1 || t === 'hg';
        if (selectedGame === 'E') return t.indexOf('soulsilver') !== -1 || t === 'ss';
        return true;
      });
    }
